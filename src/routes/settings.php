...
$app->get('/password-edit', function (Request $request, Response $response, array $args) {
    if (empty($_SESSION['current_user'])) {
        throw new Exception("Vous devez être connecté pour accéder à cette page");
    } else {
        return $this->view->render('settings/password-edit.html.twig', ['email' => $_SESSION['current_user']['email']]);
    }
});

$app->post('/password-edit', function (Request $request, Response $response, array $args) {
    if (empty($_SESSION['current_user'])) {
        throw new Exception("Vous devez être connecté pour accéder à cette page");
    } else {
